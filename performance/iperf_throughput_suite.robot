...
TCP Downlink (POP→DN) - 4 Streams
    ${result}=    Run Iperf TCP    pop    dn    streams=4
    Log Raw Results    TCP-Downlink-4Stream    ${result}

TCP Uplink (DN→POP) - 4 Streams
    ${result}=    Run Iperf TCP    dn    pop    streams=4
    Log Raw Results    TCP-Uplink-4Stream    ${result}

TCP Bidirectional (DN↔POP) - 4 Streams
    ${result}=    Run Iperf TCP Bidirectional    pop    dn    streams=4
    Log Raw Results    TCP-Bidirectional-4Stream    ${result}


TCP Downlink (POP→DN) - 1 Stream
    ${result}=    Run Iperf TCP    pop    dn    streams=1
    Log Raw Results    TCP-Downlink-1Stream    ${result}

TCP Uplink (DN→POP) - 1 Stream
    ${result}=    Run Iperf TCP    dn    pop    streams=1
    Log Raw Results    TCP-Uplink-1Stream    ${result}

TCP Bidirectional (DN↔POP) - 1 Stream
    ${result}=    Run Iperf TCP Bidirectional    pop    dn    streams=1
    Log Raw Results    TCP-Bidirectional-1Stream    ${result}


UDP Downlink (POP→DN)
    ${result}=    Run Iperf UDP    pop    dn
    Log Raw Results    UDP-Downlink    ${result}

UDP Uplink (DN→POP)
    ${result}=    Run Iperf UDP    dn    pop
    Log Raw Results    UDP-Uplink    ${result}

UDP Bidirectional (DN↔POP)
    ${result}=    Run Iperf UDP Bidirectional    pop    dn
    Log Raw Results    UDP-Bidirectional    ${result}
...
Log Raw Results
    [Arguments]    ${test_name}    ${iperf_output}
    ...
    ${graph_file}=    Set Variable    ${RESULT_DIR}/${test_name}_graph.png
    ...
    ${row}=    Set Variable
    ...    ${timestamp},${PTP_SETUP},${test_name},${sent_value},${recv_value},${status},${POP_VERSION},${DN_VERSION}\n
    ...
    IF    not ${file_exists}
        Create File
        ...    ${csv_file}
        ...    timestamp,board_model,test_name,sent_avg,recv_avg,status\n
    END
...
Full Setup
    Log To Console    ===== Starting Full Setup =====
    Initialize Selected Models
    Initialize Run Directory
    Lock Bridge Ports For PTP
